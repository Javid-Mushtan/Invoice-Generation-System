        ...
        <div className="template1 border rounded mx-auto my-4 px-sm-4 py-3 w-100">
                ...
                {data.shippingName && data.shippingPhone && data.shippingAddress && (
                    <div className="col-md-6">
                        <div className="p-3 rounded h-100 billing-box">
                            ...
                            <p className="mb-1"><strong>{data.shippingPhone}</strong></p>
                            <p className="mb-1"><strong>{data.shippingAddress}</strong></p>
                        </div>
                    </div>
                )}
                <div className="col-md-6">
                    <div className="p-3 rounded h-100 billing-box">
                        ...
                        <p className="mb-1"><strong>{data.billingAddress}</strong></p>
                        <p className="mb-1"><strong>{data.billingPhone}</strong></p>
                    </div>
                </div>
            </div>
                    ...
                    <table className="table table-bordered">
                        ...
                        {data.items.map((item, index) => (
                            <tr key={index}>
                                ...
                                <td className="p-2 text-end">Rs {Number(item.amount || 0).toFixed(2)}</td>
                                <td className="p-2 text-end">Rs {(item.qty * item.amount).toFixed(2)}</td>
                            </tr>
                        ))}
                        </tbody>
                    </table>
            ...
            {(data.accountName || data.accountNumber || data.accountIfscCode) && (
                <div className="mt-4">
                    ...
                    {data.accountIfscCode && <p className="mb-1"><strong>Account IFSC Code: </strong>{data.accountIfscCode}</p>}
                </div>
            )}
